...
本技能通过腾讯云SCF代理（域名 ap-guangzhou.tencentscf.com）并发转发请求到以下OTA平台API：
- 飞猪、途牛、同程、美团、RollingGo
- **传输数据**：仅发送城市名、入住/离店日期、搜索关键词等查询条件
- **不传输**：不收集用户个人信息、不发送身份数据、不记录搜索历史
- 代理仅做透明转发，不存储任何用户数据，不记录请求内容
